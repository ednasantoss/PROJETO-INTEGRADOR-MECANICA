...
function configurarModal() {
    ...
    formCarro.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:9090/api/veiculos/cadastro', {
                method: 'POST',
...
async function carregarVeiculos() {
    ...
    try {
        const response = await fetch(`http://localhost:9090/api/veiculos?id_cliente=${cliente.id_cliente}`);
